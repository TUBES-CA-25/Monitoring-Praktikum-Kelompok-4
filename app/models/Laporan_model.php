...
public function getRekapKehadiran($id_tahun = null) {
    // Perbaikan: Mengganti j.nama_jurusan menjadi j.jurusan sesuai error database
    $query = "SELECT f.*, m.nama_matkul, t.tahun_ajaran, k.kelas, r.nama_ruangan,
                     ...
                     (SELECT COUNT(*) FROM trs_mentoring WHERE id_frekuensi = f.id_frekuensi AND status_dosen = 1) as hadir_dosen,
                     (SELECT COUNT(*) FROM trs_mentoring WHERE id_frekuensi = f.id_frekuensi AND status_asisten1 = 1) as hadir_asisten1,
                     (SELECT COUNT(*) FROM trs_mentoring WHERE id_frekuensi = f.id_frekuensi AND status_asisten2 = 1) as hadir_asisten2
              ...
              JOIN mst_asisten a1 ON f.id_asisten1 = a1.id_asisten
              JOIN mst_asisten a2 ON f.id_asisten2 = a2.id_asisten";
    ...
    $this->db->query($query);
    if ($id_tahun) $this->db->bind('id_tahun', $id_tahun);
